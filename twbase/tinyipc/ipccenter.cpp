...
bool IpcCenter::closeLink(IIpcLink* l)
{
    ...
    TwScopeLovkV1 lock(&m_remoteHandleLock);
...
LRESULT IpcCenter::_ipcLogicProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_IPC_MSG:
        {
            std::auto_ptr<IPCMsgBin> ipcMsgBin((IPCMsgBin*)lParam);
...
unsigned int IpcCenter::_ipcThreadProc()
{
    ...
    TwUtils::customizeUIPIPolicy(m_ipcWindow, WM_COPYDATA, true);
    TwUtils::customizeUIPIPolicy(m_ipcWindow, WM_IPC_CONNECTLINK, true);
    TwUtils::customizeUIPIPolicy(m_ipcWindow, WM_IPC_CONNECTLINKACK, true);
    TwUtils::customizeUIPIPolicy(m_ipcWindow, WM_IPC_CLOSELINK, true);
    ...
    while(running)
    {
        ...
        {
            DAssert_M(false, "_ipcThreadProc wait error!");
        }
    }
...
bool IpcHostCenter::_addClient(HWND window)
{
    ...
    for(std::map<std::string, IpcLink*>::iterator it = m_links.begin(); it != m_links.end(); ++it)
    {
        ...
        {
            DAssert(it->second->remoteIdentity() == processId);
            return true;
        }
    }
